...
<section>
    <div class="card">
        ...
        @else
            <form action="{{ route('update_penyewa', $data->id) }}" class="d-inline" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
        ...
        @if ($data->status != '0')
                                        <button disabled = "disabled" class="btn btn-secondary">Selesai</button>
                                    @else
                                        <a href="{{ route('calon_penyewa.edit', $data->id) }}" class="btn btn-primary">Periksa</a>
                                    @endif
