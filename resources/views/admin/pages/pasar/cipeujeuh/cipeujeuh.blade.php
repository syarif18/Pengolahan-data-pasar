...
<div class="form-group d-flex justity-content-between">
    <div class="col-sm-4">
        <form action="/cipeujeuh" method="GET">
            @csrf
            @method('post')
    ...
    <div class="ms-2">
        <form action="{{ route('exportCipeujeuh') }}" method="POST">
            @method('post')
            @csrf
    ...
    <div class="ms-2">
        <form action="{{ route('cipeujeuhpdf') }}" class="d-inline" method="POST" target="_blank">
            @method('post')
            @csrf
            <input type="hidden" name="nama_pasar">
            <input type="hidden" name="search" value="{{ !empty($search)?$search:'' }}">
