...
<div class="form-group d-flex justity-content-between">
    <div class="col-sm-4">
        <form action="/babakan" method="GET">
            @csrf
            @method('post')
    ...
    <div class="ms-2">
        <form action="{{ route('exportBabakan') }}" method="POST">
            @method('post')
            @csrf
    ...
    <div class="ms-2">
        <form action="{{ route('babakanpdf') }}" class="d-inline" method="POST" target="_blank">
            @method('post')
            @csrf
            <input type="hidden" name="nama_pasar">
            <input type="hidden" name="search" value="{{ !empty($search)?$search:'' }}">
